...
class Auth{
            ...
            if(password_verify($Password, $user['Password'])){
                if(!empty($user['TOTP_Secret'])){
                    ...
                    if($OTP == $TOTP){
                        ...
                        $this->Priv_Level = $user['Priv_Level'];
    ...
    Usage:
    Auth::Create_User($Username(STRING), $Password(STRING), $Email(STRING), $First_Name(STRING), $Last_Name(STRING), $Priv_Level(INT))
    Returns: TRUE or FALSE    
    ---------------------------------------------------*/
    public static function Create_User($Username, $Password, $Email, $First_Name, $Last_Name, $Priv_Level){
        //Set Variables
        global $pdo;
        $Password = password_hash($Password, PASSWORD_DEFAULT);
        try{
            //Prepare SQL
            $stmt = $pdo->prepare("INSERT INTO users (Username, Password, Email, First_Name, Last_Name, Priv_Level) VALUES (?, ?, ?, ?, ?, ?)");
            //Execute SQL
            $stmt->execute([$Username, $Password, $Email, $First_Name, $Last_Name, $Priv_Level]);
    ...
    /*-------------------------------------------------
    TGNZ Auth
    Change User Priv Level
    GLOBAL $pdo MUST BE SET
    Usage:
    Auth::Change_User_Priv_Level($Username(STRING), $Priv_Level(INT))[$Username may be Username or Email]
    Returns: TRUE or FALSE
    ---------------------------------------------------*/
    public static function Change_User_Priv_Level($Username, $Priv_Level){
        //Set Variables
        global $pdo;
        try{
            //Prepare SQL
            $stmt = $pdo->prepare("UPDATE users SET Priv_Level = ? WHERE Username OR Email = ?");
            //Execute SQL
            $stmt->execute([$Priv_Level, $Username]);
            //Check if Logger class exists
            if(class_exists('Logger')){
                //Log
                logger::info('User Priv Level Changed');
            }
            //Return True
            return true;
        }
        //Catch Errors
            ...
            //Log Error and return false
            if(class_exists('Logger')){
                logger::error('User Priv Level Change Failed' . $e->getMessage());
            }
            return false;
        }
    }
